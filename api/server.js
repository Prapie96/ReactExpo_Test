  ...
  app.post("/img",async(req,res)=>{
    ...
    if(userid){
      con.query(filenameimg,[userid],(err,result)=>{
        res.send({uri: 'http://192.168.1.106:3000/img/'+ result[0].img});
      })
...
app.put('/edit', async(req,res)=>{
   ...
   //const {userid} = req.body;
    ...
    if(firstname && lastname && nickname){
      ...
      console.log("Getuserinput",firstname,lastname,nickname);
      console.log("id put",userid);
      const changeuser = "UPDATE userinfo SET firstname = ?,lastname = ?,nickname = ? WHERE userid = ? ";
      con.query(changeuser,[firstname,lastname,nickname,userid],(err,result)=>{
      ...
      console.error('firstname or lastname or nickname not value')
    }
...
app.delete('/deleteuser',async(req,res)=>{
 ...
 if(userid> 0){
  ...
  //   const sql = "UPDATE userinfo SET img = ? WHERE userid = ?";
  //   if(imageName){
  //     con.query(sql,[imageName,userid],(err,result)=>{
  //       if(err){
  //       throw(err);
  //     }
  //     if(result.affectedRows === 1){  
